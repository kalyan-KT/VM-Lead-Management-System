...
interface WebsiteLeadDetailProps {
    lead: Lead | null;
    open: boolean;
    onClose: () => void;
...
export function WebsiteLeadDetail({ lead, open, onClose, onUpdateStatus, onDelete }: WebsiteLeadDetailProps) {
    if (!lead) return null;
        ...
        <Sheet open={open} onOpenChange={onClose}>
            <SheetContent className="w-full sm:max-w-md overflow-y-auto">
                ...
                <div className="space-y-6">
                    ...
                    {/* Project Details */}
                    <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-3 uppercase tracking-wider">Project</h3>
                    ...
                    <div>
                        ...
                        <div className="bg-muted/30 p-4 rounded-md text-sm whitespace-pre-wrap">
                            {lead.contextNote ? lead.contextNote.replace('Message: ', '').split('\nPhone')[0] : 'No message provided.'}
                        </div>
                    </div>
